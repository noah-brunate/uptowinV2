...
@app_views.route('/users/<string:user_id>/reviews',
...
@app_views.route('/users/<string:user_id>/reviews', methods=['POST'],
                 strict_slashes=False)
@swag_from('documentation/reviews/post.yml', methods=['POST'])
def create_obj_review(user_id):
    """ create new instance """
    use = storage.get(User, user_id)
    if use is None:
        abort(404)
...
def post_review(review_id):
    ...
    for key, value in request.get_json().items():
        if key not in ['id', 'user_id', 'created_at', 'updated']:
